...
import { serverClient } from "@/app/_app/serverClient";
import { useEffect, useState } from "react";
import InputAlert from "@/app/components/InputAlert";
    ...
    <section className="my-8 dark:bg-gray-800 dark:text-gray-100">
      ...
      <div className="container flex flex-col items-center justify-center mx-auto lg:flex-row lg:flex-wrap lg:justify-evenly lg:px-10">
        <div className="flex flex-col max-w-sm mx-4 my-6 shadow-lg">
          {loading ? (
            <div className="px-56 py-48 rounded-t-lg sm:px-8 md:px-12 bg-gray-900">
              <Loader />
            </div>
          ) : (
            <div className="px-4 py-12 rounded-t-lg sm:px-8 md:px-12 dark:bg-gray-900">
              {valid ? (
                <p className="relative px-6 py-1 text-lg italic text-center dark:text-gray-100">
                  ...
                  <span className="text-violet-400">{valData?.name}</span> for
                  the 100x Devs Opensource Cohort on 10th November 2023
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                    fill="currentColor"
      ...
      <Link
        href={`${process.env.NEXT_PUBLIC_CERTIFY_URL}/certify`}
